...
void MigrationIndexHash::transportSnapshot(int part_id) {
	auto iter = getBeginIterator(part_id);
	while(!iter->isEnd()) {
		pair<uint64_t, row_t*> pair = *iter;
		++iter;
...
// 			continue;
// 		}
// 		MigrationBucketHeader * cur_bkt = (*_buckets)+i;
// 		cout << "第"<< i << "个bucket:{";
// 		MigrationBucketNode *cur_node = cur_bkt->first_node;
// 		while (cur_node) {
// 			printf("key:%ld ,item:[", cur_node->key);
// 			itemid_t * _cur_item = cur_node->items;
// 			while(_cur_item) {
// 				printf("%d ",_cur_item->valid);
