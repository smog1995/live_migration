...
void WorkerThread::process(Message * msg) {
  ...
  uint64_t starttime = get_sys_clock();
        ...
        rc = process_rqry(msg);
				break;
			case RQRY_CONT:  //  由txn_table的restart_txn创建的消息
        ...
        rc = process_rack_rfin(msg);
				break;
			case RTXN_CONT:  //  由txn_table的restart_txn创建的消息
...
RC WorkerThread::process_rtxn(Message * msg) {
        ...
        if(msg->get_rtype() == CL_QRY) {
          ...
          txn_man->register_thread(this);  //  这里的注册，似乎意味着该txn与该txn_manager绑定了同个线程
...
RC WorkerThread::process_log_migration(Message * msg) {
  LogRecord * record = logger.createRecord(&((LogMigrationMessage*)msg)->record);
  printf("lsn:%ld txn_id:%ld key:%ld\n", record->rcd.lsn, record->rcd.txn_id, record->rcd.key);
  return RCOK;
}
