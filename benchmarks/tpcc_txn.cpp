...
RC TPCCTxnManager::run_txn_state() {
    TPCCQuery* tpcc_query = (TPCCQuery*) query;
    uint64_t w_id = tpcc_query->w_id;  //  仓库号
    uint64_t d_id = tpcc_query->d_id;  //  区域号
    uint64_t c_id = tpcc_query->c_id;  //  客户id
    uint64_t d_w_id = tpcc_query->d_w_id;  //  区域所在仓库号
    uint64_t c_w_id = tpcc_query->c_w_id;  //  客户所在仓库id
    uint64_t c_d_id = tpcc_query->c_d_id;  //  客户所在区域id
    char * c_last = tpcc_query->c_last;    
    double h_amount = tpcc_query->h_amount;//  历史金额
    bool by_last_name = tpcc_query->by_last_name;
    bool remote = tpcc_query->remote;  //  是否为远程查询
    ...
    uint64_t ol_number = next_item_id;  //  订单行编号
...
//  1.查找条件为客户的c_w_id,c_d_id和c_last（姓氏)
  ...
  //  2. 查找条件为客户的c_w_id,c_d_id和c_id（姓氏)
  	...
  	/*======================================================================+
   		...
   		WHERE c_w_id = :c_w_id AND c_d_id = :c_d_id AND c_id = :c_id;
   	+======================================================================*/
	RC rc  = get_row(r_cust, WR, r_cust_local); //  首先获取该行的位置放到r_cust_local中
